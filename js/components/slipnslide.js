(function($) {
    ...
    var cache   = {
        ...
        aux     = {
            ...
            selectSlice     : function( $el, $slices, $navNext, $navPrev, settings ) {
                ...
                // console.log('selectSlide def');
                // if($el instanceof jQuery) { console.log('jQuery object'); }
                // else { console.log(typeof($el)); }
                ...
                return $.Deferred(
                    function( dfd ) {
                        ...
                        if( expanded ) {
                            ...
                            // removing the ss-expanded class to shrink back the container
                            $el.find('.ss-container').removeClass('ss-expanded');
                        ...
                        // if it's closed..
                            ...
                            cache.idx_expanded  = $el.parent('.slide').index();
                            ...
                            $el.find('.ss-container').addClass('ss-expanded');
                            // control the navigation buttons visibility
                        ...
                        $el.stop()
                           .animate( animParam, settings.animSpeed, settings.animEasing, function() {
                                if( !expanded ) {
                                    $el.find('.ss-content').fadeIn( settings.contentAnimSpeed );
                                }
                           })
                           .find('.ss-title')
                           .stop()
            ...
            navigate        : function( dir, $slices, $navNext, $navPrev, settings ) {
                ...
                if( cache.idx_expanded != -1 && !settings.savePositions ) {
                    ...
                    console.log('if statement inside navigate()');
                    console.log($el);
            ...
            slide           : function( dir, $slices, $navNext, $navPrev, settings ) {
                ...
                ( dir === 1 ) ? $nextSlice = $currentSlice.parent().next().children('.ss-slice') : $nextSlice = $currentSlice.parent().prev().children('.ss-slice');
                
                $slices.each(function(i){
                    var $slice  = $(this);
                    $slice.find('.ss-container').removeClass('ss-expanded');
                });
                ...
                if( ( dir === 1 && !aux.canSlideDown( $slices, settings ) ) || 
                    ...
                    console.log('if statement inside slide()');
                ...
                $slices.each(function(i) {
                    ...
                    if( !settings.savePositions || cache.idx_expanded === -1 )
                        ...
                        if( i === $nextSlice.parent().index() ) {
                            $slice.data( 'expanded', true );
                            $slice.find('.ss-container').addClass('ss-expanded');
                            cache.idx_expanded  = $slice.parent().index();
                            itemHeight          = settings.expandedHeight;
                            ...
                            $slice.stop()
                                  .animate({
                                        ...
                                        $slice.find('.ss-content').fadeIn( settings.contentAnimSpeed );
                                  })
                                  .find('.ss-title')
                                  .stop()
                                  .animate({
            ...
            canSlideDown    : function( $slices, settings ) {
                ...
                if( $last.parent().index() !== cache.totalSlices - 1 )
                    return true;
            }
        },
        methods = {
            init        : function( options ) {
                ...
                if( this.length ) {
                    ...
                    return this.each(function() {
                        ...
                        $el.prepend('<div class="ss-nav"><span class="ss-nav-prev"></span><span class="ss-nav-next"></span></div>');
                            // the accordion's slices
                        var $slices         = $el.find('div.ss-slice'),
                            // the navigation buttons
                            $navNext        = $el.find('span.ss-nav-next'),
                            $navPrev        = $el.find('span.ss-nav-prev');
                        ...
                        // click event
                        $slices.bind('click.slipnslide', function(e) {
                            $slices.each(function(i){
                                var $slice  = $(this);
                                $slice.find('.ss-container').removeClass('ss-expanded');
                            });
                            ...
                            if( settings.visibleSlices > 1 ) {
                                var $el         = $(this);
                                // console.log('bind on slice click');
                                // if($el instanceof jQuery) { console.log('jQuery object'); }
                                // else { console.log(typeof($el)); }
                        ...
                        // navigation events
                        $navNext.bind('click.slipnslide', function(e){
                            aux.navigate( 1, $slices, $navNext, $navPrev, settings );
                        });
                        
                        $navPrev.bind('click.slipnslide', function(e){
                            aux.navigate( -1, $slices, $navNext, $navPrev, settings );
                        });                                             
                    });
                }
    ...
    $.fn.slipnslide = function(method) {
        if ( methods[method] ) {
            return methods[method].apply( this, Array.prototype.slice.call( arguments, 1 ));
        } else if ( typeof method === 'object' || ! method ) {
            return methods.init.apply( this, arguments );
        } else {
            $.error( 'Method ' +  method + ' does not exist on jQuery.slipnslide' );
        }
    };
